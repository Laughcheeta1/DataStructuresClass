...
public class AllSolutions {
	...
	public static void queensProblem()
	{
		...
		{
			putTheQueens(columnTraveler, 0, board);
			columnTraveler++;
		}
	}
	
	/**
	 * Recieves the column and the row where to put the next queen. This is the method that makes the magic happen
	 * 
	 * @param position
	 * @param board
	 * @return confirmation
	 */
	private static void putTheQueens(int column, int row, int[][] board)
	{
		...
		while (columnTraveler < chessSize)
		{
			...
			putTheQueens(columnTraveler, row + 1, board);
			columnTraveler++;
		}
	...
	{
		if (seeIfColumnUnderAttack(column, 0, board) || seeIfBottomLeftDiagonal(column, row, board) || seeIfBottomRightDiagonal(column, row, board))
		{
			return true;
		}
